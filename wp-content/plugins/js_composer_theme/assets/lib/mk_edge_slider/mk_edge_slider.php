<?php
/**
 * Edge Slider main shortcode file
 *
 * @version     5.1
 * @package     artbees
 */
...
$button2_txt_color = $button1_txt_color = $outline1_hover_color = '';
$outline2_hover_color = $button2_bg_color = $button1_bg_color = '';
$outline2_active_color = $video_color_mask_css = $outline1_active_color = '';
$pagination_class = $content_opacity = $overlay_opacity_ie = $header_skin = '';
...
// $style = ( $full_height == 'true' ) ? $height . 'px' : ''; /* AM-2133 */
$class = $el_class;
$class .= ' ' . $visibility;

?>

<div class="mk-edge-wrapper <?php echo $class; ?> js-el"
	style="<?php if ( ! $full_height ) { echo 'height:' . $height . 'px;'; }?>"
	...
	<div class="clipper">
		<div id="mk-edge-slider-<?php echo $id ?>" class="mk-slider mk-edge-slider   js-el"
			...
			data-edgeSlider-config='{
				"lazyload" : "<?php echo $lazyload_logic ?>",
			...
			>
				...
				<?php if ( $type == 'video' ) { ?>
							...
							<?php if ( ! empty( $video_preview ) ) { ?>
								<?php
									$video_bg_image_set = ( ! empty( $video_preview ) ) ? Mk_Image_Resize::get_bg_res_set( $video_preview ) : '';

									$video_preview_id = mk_get_attachment_id_from_url( $video_preview );
								if ( ! empty( $video_preview_id ) ) {
									// try to get native image resizer
									$thumbnail = Mk_Image_Resize::resize_by_id( $video_preview_id, 'landscape-mobile' );
								} else if ( ! empty( $video_preview ) ) {
									// if image is not stored locally in the server, we need to resize it via bfi_thumb
									$thumbnail = Mk_Image_Resize::resize_by_url( $video_preview, 200, 125, true, false );
								} else {
									$thumbnail = '';
								}
								?>
								<div data-thumb="<?php echo $thumbnail; ?>" class="mk-video-section-touch" <?php echo $video_bg_image_set; ?> ></div>
							<?php } ?>
						...
						<?php } else {
							...
							$slide_image_id = mk_get_attachment_id_from_url( $slide_image );

	if ( ! empty( $slide_image_id ) ) {
		// try to get native image resizer
		$thumbnail = Mk_Image_Resize::resize_by_id( $slide_image_id, 'landscape-mobile' );
	} else if ( ! empty( $slide_image ) ) {
		// if image is not stored locally in the server, we need to resize it via bfi_thumb
		$thumbnail = Mk_Image_Resize::resize_by_url( $slide_image, 200, 125, true, false );
	} else {
		$thumbnail = '';
	}
						?>
							<div data-thumb="<?php echo $thumbnail; ?>" class="mk-section-image <?php echo $animation_effect ?>"  <?php echo $bg_image_css . ' ' . $bg_image_set ?>></div>
						<?php }// End if().
	?>
