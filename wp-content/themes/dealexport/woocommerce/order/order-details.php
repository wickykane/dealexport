          ...
          <td class="product-name">
            ...
            if ($_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted()) {
              ...
              foreach ($download_files as $download_id => $file) {
                ...
                $links[] = '<small><a href="' . esc_url($file['download_url']) . '">' . sprintf(__('Download file%s', 'dealexport'), (count($download_files) > 1 ? ' ' . $i . ': ' : ': ')) . esc_html($file['name']) . '</a></small>';
              }
...
<table class="profile-table">
  <tbody>
    <tr>
      ...
      <td>
        <?php if (!empty($order->billing_email)) { ?>
          <strong><?php _e('Email:', 'dealexport'); ?></strong>&nbsp;<?php echo $order->billing_email; ?><br />
        <?php } ?>
        <?php if (!empty($order->billing_phone)) { ?>
          <strong><?php _e('Phone:', 'dealexport'); ?></strong>&nbsp;<?php echo $order->billing_phone; ?>
        <?php } ?>
    ...
    <?php } ?>
    <?php
    $note = ThemedbWoo::getNote($order->id);
    if (!empty($note)) {
    ?>
      <tr>
