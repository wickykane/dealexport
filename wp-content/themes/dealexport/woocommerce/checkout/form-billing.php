    ...
    <div class="woocommerce-billing-fields section-content content active">
        ...
        if (is_user_logged_in()) { ?>
            <div class="billing-user">
                <div>
                    <?php _e('Logged in as ', 'dealexport'); ?>
                    <?php printf('<a href="%s">%s</a>', get_author_posts_url($current_user->ID), $current_user->user_login); ?>
                </div>
                <div>
                    <?php _e('It is not you ?', 'dealexport'); ?>
                    <?php printf('<a class="billing-logout-btn" href="%s">%s</a>', wp_logout_url(wc_get_page_permalink('shop')), __('Logout', 'dealexport')); ?>
                </div>
                <div class="logout-instruction"><?php _e('If you log out now, your shopping cart will be emptied.', 'dealexport'); ?></div>
            </div>
