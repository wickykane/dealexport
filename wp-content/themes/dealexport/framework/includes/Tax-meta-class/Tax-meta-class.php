...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
   ...
   * @param array $meta
   * @since 1.0
   * @since 6.0.1 Replace `isset()` with `empty()` to prevent warning.
   * @access public
   */
  ...
  if (!isset($value['id']))
    $value['id'] = '';
	$value['url'] = ! empty( $value['src'] ) ? $value['src'] : $value['url'];
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".__('Remove Image', 'mk_framework')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".__('Upload Image', 'mk_framework')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
