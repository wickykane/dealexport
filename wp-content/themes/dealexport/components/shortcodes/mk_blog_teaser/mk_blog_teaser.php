<?php
	...
	$output .= '<section class="mk-blog-teaser ' . $class . '">';
	...
	if ( $r->have_posts() ) :
		...
		while ( $r->have_posts() ) :
			...
			$output .= '<article id="teaser-entry-' . get_the_ID() . '" class="blog-slideshow-entry swiper-slide">';
				...
				$output .= '<div class="thumb-featured-image"><a title="' . the_title_attribute(
					array(
						'echo' => false,
					)
			...
			if ( 'true' == $mk_options['blog_single_comments'] ) {
				if ( get_post_meta( get_the_ID(), '_disable_comments', true ) != 'false' ) {
					ob_start();
					comments_number( '0', '1', '%' );
